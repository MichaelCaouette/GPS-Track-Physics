...
class TrackSelect(egg.gui.Window):
    ...
    def __init__(self, name="TrackSelect", size=[1000,500]): 
        ...
        # TODO For 3D, could use this:
        # egg.pyqtgraph.opengl.GLLinePlotItem()
        # For now use the matplolib
        # The whole 3D trajectory
        self.fig_3D_all = plt.figure()
        self.ax_3D_all = self.fig_3D_all .add_subplot(111, projection='3d')
        # The chopped version (will be in a separated script soon)
    ...
    def _button_chop_clicked(self):
        ...
        self.tim_chopped = self.t  [self.chop_min:self.chop_max]
    ...
    def _update_plots(self):
        ...
        self.plot_groundproj.plot(self.lat, self.lon, pen=(255,0,0))#, name="Data") 
        self.plot_groundproj.setLabel('bottom', "Latitude", units='deg')
        self.plot_groundproj.setLabel('left', "Longitude", units='deg')        
        # The elevation VS time
        self.plot_elev.clear()
        self.plot_elev.plot(self.t/60, self.ele, pen=(255,0,0))#,  name="Data") 
        self.plot_elev.setLabel('bottom', "Elapsed time", units='s')
        self.plot_elev.setLabel('left', "Elevation", units='m')
                 
            
        # 3D trajectory        

        self.ax_3D_all.clear()
        self.ax_3D_all.set_title('Whole trajectory\n'
                                 +self.filename + '  Starting at ' 
                                 + str(self.t_init_dt)) 
        # The line for the trajectory
